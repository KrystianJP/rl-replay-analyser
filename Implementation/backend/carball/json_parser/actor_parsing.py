...
CAR_DATA_DICT_PAIRS = {
    **RB_STATE_DICT_PAIRS,
    ...
    'boost_raw': ("TAGame.Car_TA:ReplicatedBoost",), 
    'boost_active': ("TAGame.Vehicle_TA:bReplicatedBoostOn", "TAGame.Car_TA:bReplicatedBoostOn", "TAGame.Car_TA:ReplicatedBoost"),
}
...
class CarActor:
    ...
    def get_data_dict(actor_data: Dict) -> Dict:
        ...
        # --- NEW LOGIC TO HANDLE MODERN BOOST DICTIONARY ---
        if 'boost_raw' in data_dict:
            val = data_dict['boost_raw']
            if isinstance(val, dict):
                # Extract the actual number from the dict {"amount": 255}
                data_dict['boost'] = val.get('amount', val.get('boost_amount', 0))
                # If boost_active isn't set, try to get it from the same dict
                if data_dict.get('boost_active') is None:
                    data_dict['boost_active'] = val.get('active', 0) % 2 == 1
            else:
                data_dict['boost'] = val
            data_dict.pop('boost_raw') # Clean up
