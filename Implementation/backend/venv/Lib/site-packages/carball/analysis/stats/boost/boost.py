...
class BoostStat(BaseStat):
    ...
    def calculate_player_stat(self, player_stat_map: Dict[str, PlayerStats], game: Game, proto_game: game_pb2.Game,
        ...
        for player_key, stats in player_stat_map.items(): 
            ...
            player_name = player_map[player_key].name
            if 'boost' in player_data_frame.columns:
                unique_vals = player_data_frame['boost'].unique()
                print(f"DEBUG 3 (Analysis Input): Player {player_name} has {len(unique_vals)} unique boost values. Samples: {unique_vals[:5]}")
            else:
                print(f"DEBUG 3 (CRITICAL): Player {player_name} DataFrame is MISSING the 'boost' column!")
