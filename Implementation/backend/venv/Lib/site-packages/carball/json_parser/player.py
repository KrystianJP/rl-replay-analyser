...
class Player:
    ...
    def parse_player_stats(self, player_stats: dict):
        ...
        self.is_bot = player_stats.get("bBot", False)
        
        # Handle Platform safely
        platform_data = player_stats.get("Platform", {})
        self.platform = platform_data.get("value") if isinstance(platform_data, dict) else platform_data

        # --- THE CRITICAL ID FIX ---
        # 1. Try to get Epic ID first (Modern Standard)
        epic_id = player_stats.get('PlayerID', {}).get('fields', {}).get('EpicAccountId')
        
        # 2. Try to get standard OnlineID
        standard_id = player_stats.get("OnlineID")

        if epic_id:
            self.online_id = str(epic_id)
        elif standard_id and str(standard_id) != "0":
            self.online_id = str(standard_id)
        else:
            # If still 0, it's likely a bot or a legacy ID
            self.online_id = str(standard_id)

        logger.debug('Created Player from stats: %s (ID: %s)', self.name, self.online_id)
