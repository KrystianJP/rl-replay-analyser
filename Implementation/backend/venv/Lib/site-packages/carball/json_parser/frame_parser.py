...
def parse_frames(game):
    ...
    # Find the line: return {'player_ball_data': ...} ***
    # Add this right before it:
    for p_id, p_data in parser.player_dicts.items():
        # Check a random frame (e.g., frame 500)
        sample_frame = list(p_data.keys())[500] if len(p_data) > 500 else None
        if sample_frame:
            print(f"DEBUG 2 (Dict Export): Player {p_id} frame {sample_frame} boost value: {p_data[sample_frame].get('boost')}")
    ...
    def parse_frames(self):
        ...
        for i, frame in enumerate(self.replay_frames):
            ...
            for new_actor in frame['new_actors']:
                ...
                # --- ADD THIS NUCLEAR DEBUG ---
                if handler is None and "Boost" in object_name:
                    print(f"!!! CRITICAL FAIL !!!: No handler found for {object_name} (Class: {self.actors[actor_id]['ClassName']})")
