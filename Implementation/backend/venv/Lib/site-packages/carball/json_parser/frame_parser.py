    ...
    def parse_frames(self):
        ...
        for i, frame in enumerate(self.replay_frames):
            ...
            for new_actor in frame['new_actors']:
                ...
                # FIX: Handle map-prefixed names by checking if the object_name ENDS with the key
                class_name = None
                for key, value in OBJECT_CLASSES.items():
                    if object_name.endswith(key):
                        class_name = value
                        break
...
OBJECT_CLASSES = {
    ...
    "TAGame.Default__CarComponent_Boost_TA": "TAGame.CarComponent_Boost_TA", 
    "Archetypes.CarComponents.CarComponent_Boost_TA": "TAGame.CarComponent_Boost_TA"
}
