...
class PickupAnalysis:
    ...
    def add_pickups(cls, proto_game: game_pb2.Game, data_frame: pd.DataFrame):
        ...
        for player in proto_game.players:
            ...
            # Inside the 'for player in proto_game.players:' loop ***
            print(f"DEBUG 4 (Pickup Analysis): {player.name} boost column mean before scaling: {player_vals_df['boost'].mean()}")
            # After the line: player_vals_df['boost'] /= 2.55
            print(f"DEBUG 4 (Pickup Analysis): {player.name} boost column mean after scaling: {player_vals_df['boost'].mean()}")
        return
