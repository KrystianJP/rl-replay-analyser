...
class BallDistanceStat(BaseStat):
    ...
    def calculate_player_distance_to_location(player_map: Dict[str, Player], data_frame: pd.DataFrame,
        ...
        also grabbing the closest and furthest player to those locations PER TEAM.
        """

        # 1. Calculate individual distances for every player on the pitch
        ...
        # 2. Group players by team to align with Ballchasing's per-team stats
        teams = {
            'blue': [p_id for p_id, p in player_map.items() if not p.is_orange],
            'orange': [p_id for p_id, p in player_map.items() if p.is_orange]
        }

        # 3. Initialize the output DataFrame for time stats
        # Index: player_id | Columns: closest_player (time), furthest_player (time)
        player_ball_distance_times = pd.DataFrame(
            index=list(player_map.keys()), 
            columns=['closest_player', 'furthest_player']
        ).fillna(0.0)

        delta_series = data_frame['game', 'delta']

        # 4. Calculate per-team closest/furthest durations
        for team_name, team_player_ids in teams.items():
            if not team_player_ids:
                continue

            # Slicing only this team's distances ensures we don't compare against opponents
            team_distances = player_distances_data_frame[team_player_ids]

            # idxmin/max returns the column name (player_id) for the min/max value in each row
            closest_in_team = team_distances.idxmin(axis=1)
            furthest_in_team = team_distances.idxmax(axis=1)

            # Sum up the time (delta) for each player when they held that "rank" in their team
            for p_id in team_player_ids:
                player_ball_distance_times.loc[p_id, 'closest_player'] = delta_series[closest_in_team == p_id].sum()
                player_ball_distance_times.loc[p_id, 'furthest_player'] = delta_series[furthest_in_team == p_id].sum()

        # 5. Prepare the final distance dataframe with deltas included
        player_distance_with_delta = pd.concat([
            player_distances_data_frame, 
            data_frame['game', 'delta'].rename('delta')
        ], axis=1)
