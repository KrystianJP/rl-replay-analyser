...
class Game:
    ...
    def parse_all_data(self, all_data, clean_player_names: bool) -> None:
        ...
        # --- MODERN CRASH-PROOF DEMOS ---
        self.demos = []
        demo_map = dict()
        for _demo_data in all_data.get('demos_data', []):
            # Use .get() to provide a fallback if velocity keys are missing in modern replays
            a_vel = _demo_data.get("attack_velocity", {"x": 0, "y": 0, "z": 0})
            v_vel = _demo_data.get("victim_velocity", {"x": 0, "y": 0, "z": 0})
            ...
            try:
                demo = {
                    ...
                    'attacker_vel': (a_vel.get("x", 0), a_vel.get("y", 0), a_vel.get("z", 0)),
                    'victim_vel': (v_vel.get("x", 0), v_vel.get("y", 0), v_vel.get("z", 0)),
                }

                # Safe key generation for duplicate prevention
                key = (int(a_vel.get("x", 0)) + int(a_vel.get("y", 0)) + int(a_vel.get("z", 0)) +
                       int(v_vel.get("x", 0)) + int(v_vel.get("y", 0)) + int(v_vel.get("z", 0)) + 
                       int(_demo_data['frame_number'])) # Added frame to key for better uniqueness
                
                Game.add_demo_to_map(key, demo, demo_map)
            except KeyError as e:
                logger.warning(f"Could not map player IDs for demo at frame {_demo_data.get('frame_number')}: {e}")
